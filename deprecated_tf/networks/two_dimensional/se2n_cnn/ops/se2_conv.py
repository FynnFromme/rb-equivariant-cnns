...
def se2n_se2n(input_tensor: tf.Tensor, filter: tf.Variable, periodicity: float = 2*np.pi, diskMask: bool = True, 
    ...
    Args:
        input_tensor (tf.Tensor): SE(2,N) input tensor with shape: [batch_size, h, w, orientations, in_channels].
...
def z2_dn(input_tensor: tf.Tensor, filter: tf.Variable, orientations: int, periodicity: float = 2*np.pi,
          diskMask: bool = True, strides: tuple = (1,1), padding: str = 'VALID', name: str = 'z2_dn_conv'
          ) -> tuple[tf.Tensor, tf.Tensor]:
    """Performs a group convolution lifting from Z2 to DN.
    ...
    Returns:
        tf.Tensor: The resulting DN feature maps of shape [batch_size, h', w', 2*orientations, out_channels].
        tf.Tensor: Theformated filters, i.e., the full stack of rotated/flipped filters of shape
            [2*orientations, filter_size, filter_size, in_channels, out_channels]
    """
    batch_size = tf.shape(input_tensor)[0] # batch size is unknown during construction, thus use tf.shape
    ...
    # Precompute a rotated and flipped stack of filters, shape=[2*orientations,h,w,in_channels,out_channels]
    filter_stack = rotate_lifting_filters(filter, orientations, periodicity=periodicity, diskMask=diskMask)
    filter_stack = flip_lifting_filters(filter_stack)

    # Format the filter stack as a 2D filter stack, shape=[h,w,in_channels,2*orientations*out_channels]
    filters_as_if_2D = tf.transpose(filter_stack, [1, 2, 3, 0, 4])
    filters_as_if_2D = tf.reshape(filters_as_if_2D, [filter_size_h, filter_size_w, channels_in, 2*orientations * channels_out])
    ...
    # Reshape to an SE2 image (split the orientation and out_channels axis)
    output_shape_3d = tf.concat([[batch_size], layer_output.shape[1:3], [2*orientations, channels_out]], axis=0)
    layer_output = tf.reshape(layer_output, output_shape_3d)

    return layer_output, filter_stack


def dn_dn(input_tensor: tf.Tensor, filter: tf.Variable, periodicity: float = 2*np.pi, diskMask: bool = True, 
          strides: tuple = (1,1), padding: str = 'VALID', name: str = 'dn_dn_conv') -> tuple[tf.Tensor, tf.Tensor]:
    """Performs a group convolution from DN to DN.

    Args:
        input_tensor (tf.Tensor): DN input tensor with shape: [batch_size, h, w, 2*orientations, in_channels].
        filter (tf.Variable): The filter bank of shape [filter_size, filter_size, 2*orientations, in_channels, out_channels].
        periodicity (float, optional): The range of rotations. Defaults to 2*np.pi.
        diskMask (bool, optional): Whether values outside a circular mask are set to zero. Defaults to True.
        strides (tuple, optional): The stride used in the conv operation. Defaults to (1,1).
        padding (str, optional): Padding of the conv operation ('VALID' or 'SAME'). Defaults to 'VALID'.
    ...
    Returns:
        tf.Tensor: The resulting DN feature maps of shape [batch_size, h', w', 2*orientations, out_channels].
        tf.Tensor: Theformated filters, i.e., the full stack of rotated/flipped filters of shape
            [2*orientations, filter_size, filter_size, 2*orientations, in_channels, out_channels]
    """
    
    batch_size = tf.shape(input_tensor)[0] # batch size is unknown during construction, thus use tf.shape

    # filter dimensions
    filter_size_h, filter_size_w, transformations, channels_in, channels_out = map(int, filter.shape)

    # Preparation for group convolutions
    # Precompute a rotated/flipped stack of DN filters
    # With shape: [2*orientations, h, w, 2*orientations, channels_in, channels_out]
    filter_stack = rotate_dn_conv_filters(filter, periodicity, diskMask)
    filter_stack = flip_gconv_filters(filter_stack)
    ...
    # Prepare the input tensor (merge the flip/orientation and channel axis) for the 2D convolutions:
    input_shape_2d = tf.concat([[batch_size], input_tensor.shape[1:3], [transformations*channels_in]], axis=0)
    input_tensor_as_if_2D = tf.reshape(input_tensor, input_shape_2d)
                                       

    # Reshape the filters for 2D convolutions (flip/orientation+channels_in axis are merged, 
    # flip/rotation+channels_out axis are merged)
    filters_as_if_2D = tf.transpose(filter_stack, [1, 2, 3, 4, 0, 5])
    filters_as_if_2D = tf.reshape(
        filters_as_if_2D, [filter_size_h, filter_size_w, transformations * channels_in, transformations * channels_out])
    ...
    # Reshape into an DN image (split the flip/rotation and channels_out axis)
    output_shape_3d = tf.concat([[batch_size], layer_output.shape[1:3], [transformations, channels_out]], axis=0)
    layer_output = tf.reshape(layer_output, output_shape_3d)
...
def rotate_gconv_filters(filter: tf.Tensor, periodicity: float = 2 * np.pi, diskMask: bool = True) -> tf.Tensor:
    ...
    for orientation in range(orientations):
        ...
        filters_temp = tf.roll(filters_temp, shift=orientation, axis=2)
        set_of_rotated_filters[orientation] = filters_temp

    return tf.stack(set_of_rotated_filters)


def rotate_dn_conv_filters(filter: tf.Tensor, periodicity: float = 2 * np.pi, diskMask: bool = True) -> tf.Tensor:
    """Rotates the set of DN filters. Rotation of DN filters involves planar rotations and a shift in orientation.
    Filter must have odd size.

    Args:
        filter (tf.Tensor): The filter bank of shape [filter_size, filter_size, 2*orientations, in_channels, out_channels].
        periodicity (float, optional): The range of rotations. Defaults to 2*np.pi.
        diskMask (bool, optional): Whether values outside a circular mask are set to zero. Defaults to True.

    Returns:
        tf.Tensor: The set of rotated filters of shape 
            [orientations, filter_size, filter_size, 2*orientations, in_channels, out_channels].
    """
    # Rotation of an SE2 filter consists of two parts:
    # PART 1. Planar rotation
    # PART 2. A shift in theta direction
    ...
    # Unpack the shape of the input filter
    filter_size_h, filter_size_w, transformations, channels_in, channels_out = map(int, filter.shape)
    orientations = transformations//2

    # PART 1 (planar rotation)
    # Flatten the baseline filter
    # Resulting shape: [h*w,transformations*channels_in*channels_out]
    filter_flat = tf.reshape(filter, [filter_size_h * filter_size_w, transformations * channels_in * channels_out])
    ...
    # Matrix multiplication (each 2D plane is now rotated)
    # Resulting shape: [orientations*h*w, transformations*channels_in*channels_out]
    filters_planar_rotated = tf.matmul(rotOp_matrix, filter_flat)
    filters_planar_rotated = tf.reshape(
        filters_planar_rotated, [orientations, filter_size_h, filter_size_w, transformations, channels_in, channels_out])
    ...
    for orientation in range(orientations):
        ...
        filters_temp = tf.reshape(filters_temp, [filter_size_h, filter_size_w, 2, orientations, channels_in, channels_out])
        filters_temp = tf.roll(filters_temp, shift=orientation, axis=3)
        filters_temp = tf.reshape(filters_temp, [filter_size_h, filter_size_w, transformations, channels_in, channels_out])
...
def flip_lifting_filters(filter: tf.Tensor) -> tf.Tensor:
    filter_flipped = tf.reverse(filter, axis=[2])
    return tf.concat([filter, filter_flipped], axis=0)


def flip_gconv_filters(filter: tf.Tensor) -> tf.Tensor:
    orientations, filter_size_h, filter_size_w, transformations_in, channels_in, channels_out = map(int, filter.shape)
    
    # PART 1 (planar flip)
    filter_flipped = tf.reverse(filter, axis=[2])
    
    # PART 2 (circular shift in flip direction)
    filter_flipped = tf.reshape(filter_flipped, 
                                [orientations, filter_size_h, filter_size_w, 2, orientations, channels_in, channels_out])
    filter_flipped = tf.roll(filter_flipped, shift=1, axis=3)
    filter_flipped = tf.reshape(filter_flipped, 
                                [orientations, filter_size_h, filter_size_w, 2*orientations, channels_in, channels_out])
