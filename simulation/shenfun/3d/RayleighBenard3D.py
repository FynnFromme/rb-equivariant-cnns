...
class RayleighBenard(KMM):
    ...
    def __init__(self,
                 N=(32, 32, 32),
                 domain=((0, 2*np.pi), (-1, 1), (0, 2*np.pi)),
                 Ra=10000.,
                 Pr=0.7,
                 dt=0.1,
                 bcT=(0, 1),
...
if __name__ == '__main__':
    ...
    simulation_dir = os.path.join(data_dir, 'data', f'{N[0]}_{N[1]}_{N[2]}_{RA}_{PR}_{DT}_{END_TIME}')
    os.makedirs(simulation_dir, exist_ok=True)
    file_name = os.path.join(simulation_dir, 'sim')
    ...
    c = RayleighBenard(**d)
    t, tstep = c.initialize(rand=0.1, from_checkpoint=False)
    t0 = time()
    c.solve(t=t, tstep=tstep, end_time=END_TIME)
