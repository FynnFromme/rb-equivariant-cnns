...
class RB3D_Conv(keras.Layer):
    ...
    def __init__(self, channels: int, h_ksize: int, v_ksize: int, use_bias: bool = True, 
        ...
        self.name = name+str(RB3D_Conv.count)
    ...
    def build(self, input_shape: list):
        ...
        if self.v_padding == 'SAME':
            height_padding = sum(required_padding(self.v_ksize, self.in_height, self.strides[2]))
        self.padded_in_height = self.in_height + height_padding
    ...
    def pad_inputs(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        if self.h_padding == 'SAME':        
            # shape: batch, width, depth, height, channel
